import { useState, useEffect } from "react";
import { Heart } from "lucide-react";
import {
  addToFavorites,
  removeFromFavorites,
  isRecipeFavorited,
  loadFavorites,
} from "../../utils/draftStorage";

/**
 * FavoriteButton Component
 * Toggles favorite status with localStorage support
 * Can work with either recipe object or recipeId
 */
export default function FavoriteButton({
  recipe,
  recipeId,
  onToggle,
  showCount = false,
  initialCount = 0,
  size = "md",
}) {
  const [isFavorited, setIsFavorited] = useState(false);
  const [favoriteCount, setFavoriteCount] = useState(initialCount);
  const [isAnimating, setIsAnimating] = useState(false);
  const [currentRecipe, setCurrentRecipe] = useState(null);

  // Size variants
  const sizes = {
    sm: "w-8 h-8",
    md: "w-10 h-10",
    lg: "w-12 h-12",
  };

  const iconSizes = {
    sm: "w-4 h-4",
    md: "w-5 h-5",
    lg: "w-6 h-6",
  };

  // Initialize recipe data
  useEffect(() => {
    if (recipe) {
      // If full recipe object is provided
      setCurrentRecipe(recipe);
    } else if (recipeId) {
      // If only recipeId is provided, try to find the recipe in existing favorites
      const favorites = loadFavorites();
      const existingRecipe = favorites.find((fav) => fav.id === recipeId);
      if (existingRecipe) {
        setCurrentRecipe(existingRecipe);
      } else {
        // Create minimal recipe object if not found in favorites
        setCurrentRecipe({
          id: recipeId,
          name: `Resep ${recipeId}`,
          category: "makanan", // default category
          image_url: "",
          cook_time: "0",
          average_rating: 0,
          description: "",
        });
      }
    }
  }, [recipe, recipeId]);

  // Check if recipe is favorited on mount
  useEffect(() => {
    if (currentRecipe?.id) {
      const favorited = isRecipeFavorited(currentRecipe.id);
      setIsFavorited(favorited);
    }
  }, [currentRecipe]);

  const handleToggle = async (e) => {
    e.stopPropagation(); // Prevent card click

    if (!currentRecipe?.id) {
      console.error("Recipe data is required for favoriting");
      return;
    }

    setIsAnimating(true);
    setTimeout(() => setIsAnimating(false), 300);

    let newFavoritedState;

    if (isFavorited) {
      // Remove from favorites
      newFavoritedState = false;
      removeFromFavorites(currentRecipe.id);
      setFavoriteCount((prev) => Math.max(0, prev - 1));
    } else {
      // Add to favorites
      newFavoritedState = true;
      addToFavorites(currentRecipe);
      setFavoriteCount((prev) => prev + 1);
    }

    setIsFavorited(newFavoritedState);

    // Call parent callback if provided
    if (onToggle) {
      onToggle(currentRecipe.id, newFavoritedState);
    }

    // Dispatch custom event for other components to listen to
    window.dispatchEvent(
      new CustomEvent("favoritesUpdated", {
        detail: { recipeId: currentRecipe.id, favorited: newFavoritedState },
      })
    );
  };

  // If no recipe data is available at all, don't render the button
  if (!currentRecipe?.id) {
    return null;
  }

  return (
    <button
      onClick={handleToggle}
      className={`
        ${sizes[size]} rounded-full flex items-center justify-center gap-1.5
        transition-all duration-200 
        ${
          isFavorited
            ? "bg-red-500 hover:bg-red-600 text-white"
            : "bg-white/90 hover:bg-white text-slate-700 hover:text-red-500"
        }
        backdrop-blur-sm shadow-md hover:shadow-lg
        ${isAnimating ? "scale-125" : "scale-100"}
        group
      `}
      title={isFavorited ? "Hapus dari favorit" : "Tambahkan ke favorit"}
    >
      <Heart
        className={`
          ${iconSizes[size]} 
          transition-all duration-200
          ${isFavorited ? "fill-current" : ""}
          ${isAnimating ? "animate-pulse" : ""}
        `}
      />
      {showCount && favoriteCount > 0 && (
        <span className="text-xs font-semibold">
          {favoriteCount > 999 ? "999+" : favoriteCount}
        </span>
      )}
    </button>
  );
}
